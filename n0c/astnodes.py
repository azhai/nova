...
from defs import ASTNode, NodeType, TypeKind, fatal, quote_string
...
def mk_ast_node(op: NodeType, left: Optional[ASTNode] = None,
...
def mk_ast_leaf(op: NodeType, type_: Optional['Type'] = None,
...
def dump_ast(out, node: Optional[ASTNode], level: int = 0) -> None:
    ...
    elif node.op == NodeType.A_STRLIT:
        value = quote_string(node.strlit)
        out.write(f"({value})")
    elif node.op == NodeType.A_NUMLIT:
        if node.type and node.type.kind in (TypeKind.K_FLT32, TypeKind.K_FLT64):
    ...
    elif node.op in (NodeType.A_PRINT, NodeType.A_PRINTF, NodeType.A_FUNCCALL):
        if node.left:
