...
from defs import ASTNode, NodeType, DataType, TokType, TypeKind, Sym, SymType, cast_node, fatal, ty_float64
...
class StmtProcessor:
    ...
    def print_statement(expr: ASTNode) -> ASTNode:
        ...
        if expr.type.kind == TypeKind.K_FLT32:
            target_type = TypeProcessor.get_type(TypeKind.K_FLT64)
        ...
        node = ASTNode(
            ...
            left=expr,
            right=None,
            type=TypeProcessor.get_type(TypeKind.K_VOID)
        )
        return node
    ...
    def declaration_statement(val_type: DataType, name: str, expr: Optional[ASTNode]) -> ASTNode:
        ...
        if expr:
            ...
            # 保存初始值
            if expr.op == NodeType.A_NUMLIT:
                sym.init_val = parse_litval(TokType.T_NUMLIT, expr.strlit)
            elif expr.op == NodeType.A_STRLIT:
                sym.init_val = parse_litval(TokType.T_STRLIT, expr.strlit)
...
def gen_stat_printf(first: ASTNode, expr: ASTNode) -> ASTNode:
    """ 创建打印语句AST节点 """
    if expr.type.kind == TypeKind.K_FLT32:
        expr = cast_node(expr, ty_float64)
    node = ASTNode(
        op=NodeType.A_PRINTF,
        left=first,
        right=expr
    )
    return node
