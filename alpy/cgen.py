...
from defs import DataType, TypeKind, Sym, Litval, Outfh, fatal, ASTNodeType
...
class CodeGenerator:
    ...
    def cgstrlit(self, label: int, val: str) -> None:
        ...
        print(f"data $L{label} = {{ b \"{escaped}\", b 0 }}", file=Outfh)

    def cgjump(self, l: int) -> None:
        print(f"  jmp @L{l}", file=Outfh)
    ...
    def cg_func_preamble(self, func: Sym) -> None:
        print(f"export function ${func.name}(", end="", file=Outfh)
        param = func.memb
        while param:
            qtype = self.qbetype(param.type)
            print(f"{qtype} %{param.name}", end="", file=Outfh)
            param = param.next
            if param:
                print(", ", end="", file=Outfh)
        print(") {", file=Outfh)
        print("@START", file=Outfh)

    def cg_func_postamble(self) -> None:
        print("@END", file=Outfh)
        print("  ret", file=Outfh)
        print("}", file=Outfh)
    ...
    def cgglobsym(self, s: Sym) -> None:
        ...
        if s.type.kind in (TypeKind.TY_FLT32, TypeKind.TY_FLT64):
            print(f"export data ${s.name} = {{ {qtype} {qtype}_{s.init_val.dblval}, }}", file=Outfh)
        else:
            print(f"export data ${s.name} = {{ {qtype} {s.init_val.intval}, }}", file=Outfh)

    def cgprint(self, label: int, temp: int, type_: DataType) -> None:
        qtype = self.qbetype(type_)
        print(f"  call $printf(l $L{label}, {qtype} %%.t{temp})", file=Outfh)

    def cgloadlit(self, value: Litval, type_: DataType) -> int:
        t = self.cgalloctemp()
        qtype = self.qbetype(type_)
        if type_.kind in (TypeKind.TY_FLT32, TypeKind.TY_FLT64):
            print(f"  %%.t{t} ={qtype} copy {qtype}_{value.dblval}", file=Outfh)
        else:
            print(f"  %%.t{t} ={qtype} copy {value.intval}", file=Outfh)
        return t

    def cgadd(self, t1: int, t2: int, type_: DataType) -> int:
        t = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t} ={qtype} add %%.t{t1}, %%.t{t2}", file=Outfh)
        return t

    def cgsub(self, t1: int, t2: int, type_: DataType) -> int:
        t = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t} ={qtype} sub %%.t{t1}, %%.t{t2}", file=Outfh)
        return t

    def cgmul(self, t1: int, t2: int, type_: DataType) -> int:
        t = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t} ={qtype} mul %%.t{t1}, %%.t{t2}", file=Outfh)
        return t
    ...
    def cgdiv(self, t1: int, t2: int, type_: DataType) -> int:
        ...
        qtype = self.qbetype(type_)
        if type_.is_unsigned:
            print(f"  %%.t{t} ={qtype} divu %%.t{t1}, %%.t{t2}", file=Outfh)
        else:
            print(f"  %%.t{t} ={qtype} div %%.t{t1}, %%.t{t2}", file=Outfh)
        return t

    def cgnegate(self, t: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} neg %%.t{t}", file=Outfh)
        return t_new
    ...
    def cgcompare(self, op: ASTNodeType, t1: int, t2: int, type_: DataType) -> int:
        ...
        print(f"  %%.t{t} =w {op_map[op]} {qtype} %%.t{t1}, %%.t{t2}", file=Outfh)
        return t

    def cgjump_if_false(self, t1: int, label: int) -> None:
        print(f"  jz %%.t{t1}, @L{label}", file=Outfh)

    def cgnot(self, t: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        print(f"  %%.t{t_new} =w not %%.t{t}", file=Outfh)
        return t_new

    def cginvert(self, t: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} inv %%.t{t}", file=Outfh)
        return t_new

    def cgand(self, t1: int, t2: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} and %%.t{t1}, %%.t{t2}", file=Outfh)
        return t_new

    def cgor(self, t1: int, t2: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} or %%.t{t1}, %%.t{t2}", file=Outfh)
        return t_new

    def cgxor(self, t1: int, t2: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} xor %%.t{t1}, %%.t{t2}", file=Outfh)
        return t_new

    def cgshl(self, t1: int, t2: int, type_: DataType) -> int:
        t_new = self.cgalloctemp()
        qtype = self.qbetype(type_)
        print(f"  %%.t{t_new} ={qtype} shl %%.t{t1}, %%.t{t2}", file=Outfh)
        return t_new
    ...
    def cgshr(self, t1: int, t2: int, type_: DataType) -> int:
        ...
        qtype = self.qbetype(type_)
        if type_.is_unsigned:
            print(f"  %%.t{t_new} ={qtype} shru %%.t{t1}, %%.t{t2}", file=Outfh)
        else:
            print(f"  %%.t{t_new} ={qtype} shr %%.t{t1}, %%.t{t2}", file=Outfh)
        return t_new

    def cgloadvar(self, sym: Sym) -> int:
        t = self.cgalloctemp()
        qtype = self.qbe_loadtype(sym.type)
        print(f"  %%.t{t} ={qtype} load ${sym.name}", file=Outfh)
        return t

    def cgstorvar(self, t: int, exprtype: DataType, sym: Sym) -> None:
        qtype = self.qbe_storetype(sym.type)
        print(f"  store {qtype} %%.t{t}, ${sym.name}", file=Outfh)
    ...
    def cgcast(self, t: int, type_: DataType, newtype: DataType) -> int:
        ...
        if type_.kind in (TypeKind.TY_FLT32, TypeKind.TY_FLT64):
            if newtype.kind in (TypeKind.TY_FLT32, TypeKind.TY_FLT64):
                ...
                print(f"  %%.t{t_new} ={qtype} copy %%.t{t}", file=Outfh)
            else:
                qtype = self.qbetype(type_)
                new_qtype = self.qbetype(newtype)
                print(f"  %%.t{t_new} ={new_qtype} trunc {qtype} %%.t{t}", file=Outfh)
        else:
            if newtype.kind in (TypeKind.TY_FLT32, TypeKind.TY_FLT64):
                qtype = self.qbetype(type_)
                new_qtype = self.qbetype(newtype)
                print(f"  %%.t{t_new} ={new_qtype} extend {qtype} %%.t{t}", file=Outfh)
            else:
                qtype = self.qbe_exttype(type_)
                new_qtype = self.qbetype(newtype)
                print(f"  %%.t{t_new} ={new_qtype} {qtype} %%.t{t}", file=Outfh)
        return t_new

    def cgaddlocal(self, type_: DataType, sym: Sym) -> None:
        qtype = self.qbe_storetype(type_)
        print(f"  var {qtype} {sym.name}", file=Outfh)
    ...
    def cgcall(self, sym: Sym, numargs: int, arglist: List[int], typelist: List[DataType]) -> int:
        ...
        args_str = ", ".join(args)
        print(f"  %%.t{t} =l call ${sym.name}({args_str})", file=Outfh)
        return t
